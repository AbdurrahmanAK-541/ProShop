...
export const cartReducer = (state = {  cartItems: [] }, action) => { /*cartItems Array = more than one item in Cart*/
    switch(action.type) {
        case CART_ADD_ITEM:
            const item = action.payload /*things passed through the product such as the Id which will be called product*/
            ...
            if (existItem){ /*if the item exists*/
                return{
            ...
            }else{ /*if the item doesn't exists then push it to the cartItems array*/
                return {
                    ...state,/*return a state and have what's already in the state*/
                     cartItems: [...state.cartItems, item] /* set the cartItems to an array with the current existing items plus the new items*/
                }
            }
